...
export HOME_ASSISTANT_DNS_USERNAME=$(az keyvault secret show --vault-name "kv-home-automation" --name "home-assistant-HOME-ASSISTANT-DNS-USERNAME" | jq -r '.value')
export HOME_ASSISTANT_DNS_PASSWORD=$(az keyvault secret show --vault-name "kv-home-automation" --name "home-assistant-HOME-ASSISTANT-DNS-PASSWORD" | jq -r '.value')
