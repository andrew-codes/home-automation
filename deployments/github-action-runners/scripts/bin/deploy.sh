...
export GITHUB_RUNNER_TOKEN=$(az keyvault secret show --vault-name "kv-home-automation" --name "github-action-runners-GITHUB-RUNNER-TOKEN")
...
export HOME_AUTOMATION_PRIVATE_SSH_KEY=$(az keyvault secret show --vault-name "kv-home-automation" --name "github-action-runners-HOME-AUTOMATION-PRIVATE-SSH-KEY")
mkdir -p .secrets
...
export GITHUB_ACTION_JEST_REPORTER_TOKEN=$(az keyvault secret show --vault-name "kv-home-automation" --name "github-action-runners-GITHUB-ACTION-JEST-REPORTER-TOKEN")
...
export MQTT_PASSWORD=$(az keyvault secret show --vault-name "kv-home-automation" --name "mqtt-PASSWORD")
export MQTT_USERNAME=$(az keyvault secret show --vault-name "kv-home-automation" --name "mqtt-USERNAME")
export GAMING_ROOM_GAMING_PC_MAC=$(az keyvault secret show --vault-name "kv-home-automation" --name "home-assistant-GAMING-ROOM-GAMING-PC-MAC")
