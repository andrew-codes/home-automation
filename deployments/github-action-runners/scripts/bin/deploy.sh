...
export GITHUB_RUNNER_TOKEN=$(vault kv get -format=json kv/github-action-runners | jq .data.GITHUB_TOKEN | sed -e 's/^"//' -e 's/"$//')
...
export GITHUB_ACTION_JEST_REPORTER_TOKEN=$(vault kv get -format=json kv/github-action-runners | jq .data.GITHUB_ACTION_JEST_REPORTER_TOKEN | sed -e 's/^"//' -e 's/"$//')
...
export MQTT_PASSWORD=$(vault kv get -format=json kv/mqtt | jq .data.PASSWORD | sed -e 's/^"//' -e 's/"$//')
export MQTT_USERNAME=$(vault kv get -format=json kv/mqtt | jq .data.USERNAME | sed -e 's/^"//' -e 's/"$//')
export GAMING_ROOM_GAMING_PC_MAC=$(vault kv get -format=json kv/home-assistant | jq .data.GAMING_ROOM_GAMING_PC_MAC | sed -e 's/^"//' -e 's/"$//')
