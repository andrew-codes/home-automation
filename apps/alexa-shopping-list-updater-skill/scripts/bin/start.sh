...
telepresence --swap-deployment "alexa-shopping-list-updater-skill" --docker-run --rm -t -v "$PWD/../../:/app" -p "8080:80" -p "8081:5000" "node-app:latest" yarn lerna run start/dev --scope "@ha/alexa-shopping-list-updater-skill-app" --stream <<<$DEV_MACHINE_PASSWORD
