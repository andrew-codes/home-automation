...
telepresence --swap-deployment game-cache --docker-run --rm -t -v "$PWD/../../:/app" node-app:latest yarn lerna run start/dev --scope "@ha/game-state-updater-app" --stream <<<$DEV_MACHINE_PASSWORD
