import { concat, entries, flatten, filter, flow, get, identity, map, mergeWith, pick, reduce, tap, uniqBy, zipObject } from 'lodash/fp'
...
const foreignKeys = ['genres', 'developers', 'publishers', 'features', 'series', 'ageRatings', 'source', 'completionStatus', 'platforms']
...
const messageHandler: MessageHandler = {
    ...
    handle: async (topic, payload) => {
        try {
            logger.info('Handler for topic', topic)
            ...
            const toGamesWithoutForeignKeys = flow(
                map(pick(nonForeignKeys))
            )
            ...
            const dbInserts = flow(
                prepareForDbInsert,
                map(([key, value]) => map((item => db.collection(key).updateOne({ _id: item._id }, { $set: item }, { upsert: true })))(value)),
                flatten,
            )
