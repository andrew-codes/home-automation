...
fn_log_info() { echo "$(date +"%Y-%m-%d-%H%M%S") - $APPNAME: $1"; }
fn_log_warn() { echo "$(date +"%Y-%m-%d-%H%M%S") - $APPNAME: [WARNING] $1" 1>&2; }
fn_log_error() { echo "$(date +"%Y-%m-%d-%H%M%S") - $APPNAME: [ERROR] $1" 1>&2; }
fn_log_info_cmd() {
