...
telepresence intercept "home-assistant" --service home-assistant-api --env-file intercept.env --port 8123:8123 --mount ./tmp -- /bin/bash -c 'docker run --rm -t $(cat intercept.env | sed -n "/^[^\t]/s/=.*//p" | sed "/^$/d" | sed "s/^/-e /g" | tr "\n" " ") --env HASS_SERVER="http://localhost:8123" --env "TELEPRESENCE_ROOT=/tmp" -v "$PWD/src:/home-assistant-src" -v "$PWD/tmp:/tmp" -p "8123:8123" "docker-registry:5000/home-assistant:latest"'
