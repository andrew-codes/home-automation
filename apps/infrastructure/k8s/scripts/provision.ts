...
const run = async (
  ...
  const TF_VAR_ip = await configurationApi.get("k8s/main-node/ip")
  const TF_VAR_gateway = await configurationApi.get("unifi/ip")
  const pveHost = await configurationApi.get("proxmox/host/pve")
  const TF_VAR_pm_username = await configurationApi.get("k8s/machine/username")
  const TF_VAR_pm_password = await configurationApi.get("k8s/machine/password")
  const TF_VAR_hostname = await configurationApi.get("k8s/name")
