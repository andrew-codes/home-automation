...
router_ip: $ROUTER_IP
turn_off_gaming_room_gaming_pc_command: 'ssh -n -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i /root/.ssh/id_rsa $MACHINE_USERNAME@$GAMING_ROOM_GAMING_PC_IP cmd.exe "$Env:WinDir\System32\rundll32.exe powrprof.dll,SetSuspendState Standby"'
EOL
