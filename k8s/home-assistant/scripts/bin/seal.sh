...
echo "$GAMING_ROOM_GAMING_PC_SSH_PUBLIC_KEY" >.secrets/gaming-pc.pub
echo "$GAMING_ROOM_GAMING_PC_SSH_PRIVATE_KEY" >.secrets/gaming-pc
...
router_ip: $ROUTER_IP
turn_off_gaming_room_gaming_pc_command: 'ssh -t -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i /root/.ssh/id_rsa $GAMING_ROOM_GAMING_PC_USERNAME@gaming-pc.smith-simms.family cmd.exe "%windir%\System32\rundll32.exe powrprof.dll,SetSuspendState Standby"'
EOL
